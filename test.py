...
def debug_dc( micros, dc, KP, KI, KD ):
	...
	print "t=%.3f\tx=%.4f\ty=%.4f\toffA=%.4f\terrA=%.4f\tfeatA=[%.4f %.4f %.4f]\terrA=[%.4f %.4f %.4f]" % ( micros/1000000.0, dc.get_position_x(), dc.get_position_y(), dc.get_motor_offset_a(), dc.get_err_a(), dc.get_pid_a_error_p(), dc.get_pid_a_error_i(), dc.get_pid_a_error_d(), dc.get_pid_a_error_p() * KP, dc.get_pid_a_error_i() * KI, dc.get_pid_a_error_d() * KD )
